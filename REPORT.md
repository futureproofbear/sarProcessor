...
**Date:** 2026-06-16
**Status:** Algorithm complete and verified; CPU port works; FPGA fabric is designed but not yet built
...
### 3.2 PolarFire SoC port — *CPU host complete; FPGA fabric designed, not built*
...
| AXI4-Lite register map (host↔fabric contract) | [regmap.md](mpfs/regmap.md) | Specification only |
...
- **Designed but not built:** the FPGA kernels ([fpga/*.cpp](mpfs/fpga/)) and the `FpgaBackend` body. They define the datapath and the host↔fabric contract but still require synthesis, simulation, place/route, a bitstream, and a UIO/CMA driver.

---

## 5. Remaining work to reach the Icicle Kit

In priority order (lowest risk / highest value first):

**Milestone 0 — CPU-only path on real hardware.**
Boot Linux on the Icicle Kit, install/cross-build dependencies, run `sar_pipeline.py --backend numpy --watch /data/in`. Proves I/O, parsing, geocoding, and the batch queue on silicon; delivers a working (slow) product and the golden oracle. Mostly integration, no new code.

**Milestone 1 — FFT in fabric, verified.**
Use the **Microchip CoreFFT IP** (verified, characterized) rather than the hand-written `fft1d.cpp`. Lock the datapath bit width (16- vs 18-bit) using the existing `fixedpoint.py` study against the required dynamic range. Verify each kernel with C/RTL co-simulation against the numpy golden output (smoothed-correlation + point-target PSLR check).

**Milestone 2 — Full datapath, bitstream, driver.**
- Implement the **corner-turn** transpose (the deliberately-stubbed piece in [sar_accel_top.cpp](mpfs/fpga/sar_accel_top.cpp)) — tiled BRAM/URAM so DDR access stays burst-friendly. This is the largest unwritten piece.
- Libero design: accelerator + `CoreAXI4DMAController` + AXI4-Lite, on the cache-coherent FIC port; place/route/timing-close (~150 MHz target).
- Export bitstream + device-tree overlay exposing **UIO** + a **CMA** pool.
- Implement `FpgaBackend.focus()` in [accel.py](mpfs/host/accel.py) per the handshake in [regmap.md](mpfs/regmap.md).
- Bring-up validation: `--backend fpga` vs. numpy golden, correlation → 1.0 (modulo fixed point) + PSLR.
