...
sleep 14   # let openocd examine all harts before gdb connects
"$GDB" "$ELF" -x flow_instrument.gdb 2>&1 | tr -d '\r' | grep -avE '^Reading|warranty|GPL|free soft|GNU gdb|Copyright|documentation|bug report|configured as|^Type |sifive|For help|apropos'
