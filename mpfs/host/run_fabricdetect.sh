...
"$NEW/bin/openocd.exe" -s "$NEW/openocd/scripts" --command "set DEVICE MPFS" -f board/microchip_riscv_efp6.cfg -l /c/Users/lkwangsi/Tools/openocd-new/fabricdetect.log >/dev/null 2>&1 &
echo ">>> openocd launching..."
"$GDB" "$ELF" -x flow_pipe_fabricdetect.gdb 2>&1 | tr -d '\r' | grep -avE '^Reading|warranty|GPL|free soft|GNU gdb|Copyright|documentation|bug report|configured as|^Type |sifive|For help|apropos'
echo ">>> gdb ended"
