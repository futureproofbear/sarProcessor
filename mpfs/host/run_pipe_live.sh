...
sleep 14   # let openocd finish EXAMINING all harts before gdb connects (else auto_probe race)
"$GDB" "$ELF" -x flow_pipe_live.gdb 2>&1 | tr -d '\r' | grep -avE '^Reading|warranty|GPL|free soft|GNU gdb|Copyright|documentation|bug report|configured as|^Type |sifive|For help|apropos'
cmd //c "taskkill /F /IM openocd.exe" >/dev/null 2>&1
